...
public class SearchResultController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            WebTarget webTarget = client.target(URI_WS_JDBC).path("/searchProductByNameXML");
            Response res = webTarget
                    .queryParam("search", inputSearch)
                    .queryParam("offset", offset)
                    .queryParam("limit", 10)
                    .queryParam("calo", Integer.parseInt(calo))
                    .request().get();
            String listXML = res.readEntity(String.class);
            response.setContentType("text/plain");
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write(listXML.replaceAll("\"", ""));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
